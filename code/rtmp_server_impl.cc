...
rs_errcode rtmp_server_cls::push_frame(unsigned char *frame,int frame_len)
{
	...
	{
        //fprintf(stdout,"%s no client can play!\n", __FUNCTION__);
...
//		if(m_sequence_header_len == 0)
        ...
        //fprintf(stdout,"%s f_type %d, seq_header_len %d\n", __FUNCTION__, frame[4]&0x1f, m_sequence_header_len);
		break;
	case 8: //pps
		frame_type = FT_PPS;
//		if(m_pps_len == 0)
	...
	case 5: //i frame
		...
		if(m_sequence_header_len == 0)
		{
			printf("I frame !! warning: need first sps and pps %d \n", m_sequence_header_len);
			return RS_ERRCODE_WAITSPS;
		}
		else
		{
	...
	case 1:// p or b frame
		...
		if(m_sequence_header_len == 0)
		{
			printf("P&B frame !! warning: need first sps and pps %d\n", m_sequence_header_len);
			return RS_ERRCODE_WAITSPS;
		}
		else
		{
